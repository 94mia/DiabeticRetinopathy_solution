...
def parse_args():
    parser = argparse.ArgumentParser(description='multi-fundus-disease classification options')
...
class MultiTaskClsValDataSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, item):
        return self.transform(Image.open(os.path.join(self.root, str(self.images_list[item][0])+'_'+str(self.scale_size)+'.png'))), int(self.images_list[item][1])
...
class MultiTaskClsInferenceDataSet(torch.utils.data.Dataset):
    ...
    def _scale_image(self, imagepath, scale_size):
        ...
        # image = image.crop((max(w // 2 - tw // 2, 0), max(h // 2 - th // 2, 0), min(w // 2 + tw // 2, w-1), min(h // 2 + th // 2, h-1)))
        w, h = image.size
...
class multi_task_model(nn.Module):
    def __init__(self, name, inmap, multi_classes, weights=None, scratch=False, outbincls=True):
        ...
        initialize_cls_weights(self.cls)
...
def train_bin(train_data_loader, model, criterion, optimizer, epoch, display, dme_weight_aug_ratio=1):
    ...
    logger = []
    for index, (image, label_dr) in enumerate(train_data_loader):
        data_time.update(time.time()-end)
        o_dr= model(Variable(image.cuda()))
        ...
        if index % display == 0:
            ...
            print_info = 'Epoch: [{epoch}][{iter}/{tot}]\t' \
                         ...
                         'Loss {loss.avg:.4f}\t' \
                         'DR_Accuracy {dr_acc:.4f}\t'.format(epoch=epoch, iter=index, tot=len(train_data_loader),
...
def eval_bin(eval_data_loader, model, criterion):
    ...
    logger = []
    for index, (image, label_dr) in enumerate(eval_data_loader):
        data_time.update(time.time()-end)
        o_dr = model(Variable(image.cuda()))
        loss_dr = criterion(o_dr, Variable(label_dr.cuda()))
        loss = loss_dr
        ...
        print_info = 'Eval: [{iter}/{tot}]\t' \
                     ...
                     'Data {data_time.avg:.3f}\t ' \
                     'Loss {loss.avg:.4f}\t' \
                     'DR_Accuracy {dr_acc:.4f}\t'.format(iter=index, tot=len(eval_data_loader),
                                                           batch_time=batch_time,
                                                           data_time=data_time,
                                                           loss=losses,
                                                           dr_acc=dr_accuracy
...
def infer_bin(eval_data_loader, model, refer_root, infer_result_file):
    ...
    for index, (image, imagename) in enumerate(eval_data_loader):
        ...
        data = np.column_stack((imagenames, tot_pred_dr))
...
def main():
    ...
    model = multi_task_model(opt.model, inmap=opt.crop, multi_classes=7, weights=opt.weight)
    criterion = nn.CrossEntropyLoss().cuda()
    ...
    if opt.phase == 'train':
        ...
        for epoch in range(opt.epoch):
            ...
            logger = train_bin(dataset_train, nn.DataParallel(model).cuda(), criterion, optimizer, epoch,
                                      opt.display, opt.dme_weight_aug)

            # logger_val, kp_dr, kp_dme, _,_,_,_ = eval(dataset_val, nn.DataParallel(model).cuda(), criterion)
            logger_val, acc = eval_bin(dataset_val, nn.DataParallel(model).cuda(), criterion)

            if acc > acc_best:
                print('\ncurrent best cls accuracy is: {}\n'.format(acc))
                acc_best = acc
                torch.save(model.cpu().state_dict(), os.path.join(output_dir,
                                                                  opt.dataset + '_multi_fundus_disease_' + opt.model + '_%03d' % epoch + '_best.pth'))
