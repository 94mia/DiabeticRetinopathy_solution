...
def gen_scale_image(root, imglist):
    for index in imglist:
        image = Image.open(os.path.join(root, '{}.jpg'.format(index)))
        w, h = image.size
        tw, th = (min(w, h), min(w, h))
...
list1 = glob('/home/weidong/code/dr/DiabeticRetinopathy_solution/data/zhizhen_new/LabelImages/abnormal/*.jpg')
list1 = [os.path.basename(index).split('.')[0]+'_512' for index in list1]
list2 = glob('/home/weidong/code/dr/DiabeticRetinopathy_solution/data/zhizhen_new/LabelImages/abnormal/512/*.png')
list2 = [os.path.basename(index).split('.')[0] for index in list2]
...
for index in list1:
    ...
    if index in list2:
        continue
    list3.append(index.replace('_512', ''))

print(len(list3))
print(list3)

gen_scale_image(root, list3)